...
def _guided_like_smooth(guide_gray, src, radius=12):
    sigma = max(float(radius) * 0.5, 0.5)
    out = cv2.GaussianBlur(src.astype(np.float32), (0, 0), sigma)

    edge = _normalize01(_sobel_mag(guide_gray.astype(np.float32)))
    edge = edge[..., None]
    mix = np.clip(1.0 - edge * 0.85, 0.10, 1.0)

    return (src.astype(np.float32) * (1.0 - mix) + out * mix).astype(np.float32)


def _resize_longest_side(img, max_side):
    ...
    resized = cv2.resize(img.astype(np.float32), (nw, nh), interpolation=interp)
...
def _make_strict_luma_gate(target_luma, ref_luma, local_conf, edge_safety=0.5, mode="full"):
    mode = str(mode).lower()
    ...
    if mode == "reduced":
        structure_risk = (
            edge_mismatch * 0.55 +
            detail_mismatch * 0.45
        )
        structure_risk = cv2.GaussianBlur(
            structure_risk.astype(np.float32),
            (0, 0),
            0.7 + edge_safety * 0.6,
        )

        protect = np.clip(
            structure_risk * (0.90 + 0.35 * edge_safety) +
            tg * (0.22 + 0.18 * edge_safety) +
            low_conf_inv * 0.28,
            0.0,
            1.0,
        )
        protect = cv2.GaussianBlur(
            protect.astype(np.float32),
            (0, 0),
            0.8 + 0.9 * edge_safety,
        )
    else:
...
def adaptive_chroma_fusion(
    ...
    base = t_lab.copy()
    base[..., 1:3] = _channel_stats_transfer(
        t_lab[..., 1:3],
        r_lab[..., 1:3],
        strength=global_chroma_strength,
    )

    t_proxy, proxy_scale = _resize_longest_side(t_lab, internal_max_res)
    if proxy_scale < 0.999:
        r_proxy = _resize_like(r_lab, t_proxy.shape).astype(np.float32)
    else:
        r_proxy = r_lab.astype(np.float32)

    base_proxy = t_proxy.copy()
    base_proxy[..., 1:3] = _channel_stats_transfer(
        t_proxy[..., 1:3],
        r_proxy[..., 1:3],
        strength=global_chroma_strength,
    )

    local_conf_proxy, chroma_conf_proxy, _ = _compute_confidence_maps(
        t_proxy,
        r_proxy,
    ...
    local_conf_proxy = np.clip(local_conf_proxy ** max(structure_sensitivity, 1e-4), 0.0, 1.0)

    if proxy_scale < 0.999:
        local_conf = _resize_like(local_conf_proxy, t_lab.shape).astype(np.float32)
        chroma_conf = _resize_like(chroma_conf_proxy, t_lab.shape).astype(np.float32)

        local_conf = cv2.GaussianBlur(local_conf, (0, 0), 0.8)
        chroma_conf = cv2.GaussianBlur(chroma_conf, (0, 0), 0.8)
    else:
        local_conf = local_conf_proxy.astype(np.float32)
        chroma_conf = chroma_conf_proxy.astype(np.float32)

    edge_full = _normalize01(_sobel_mag(t_lab[..., 0])).astype(np.float32)
    ...
    chroma_delta_proxy = np.zeros_like(base_proxy[..., 1:3], dtype=np.float32)
    ...
    for i, c in enumerate([1, 2]):
        ...
        if local_detail > 0.0:
            ...
            dw = np.zeros_like(src, dtype=np.float32)
        ...
        delta = (dw * local_conf_proxy + dp * chroma_conf_proxy)
        chroma_delta_proxy[..., i] = _soft_limit_delta(delta, max_chroma_shift)

    if proxy_scale < 0.999:
        chroma_delta = _resize_like(chroma_delta_proxy, t_lab.shape).astype(np.float32)
        chroma_delta = cv2.GaussianBlur(chroma_delta, (0, 0), 0.6)
    else:
        chroma_delta = chroma_delta_proxy.astype(np.float32)

    out = base.copy()
    out[..., 1:3] = base[..., 1:3] + chroma_delta * chroma_gate[..., None]
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image_ref": ("IMAGE",),
                "image_target": ("IMAGE",),
                "edge_safety": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 3.0, "step": 0.01}),
                "luma_match": ("FLOAT", {"default": 1.5, "min": 0.0, "max": 3.0, "step": 0.01}),
            }
        }
